...
//    }
//    free(member);
    self.request = [HttpRequest startRequestWithUrlString:@"http://192.168.26.37:9090/finish" httpMethod:HttpPost getParam:nil postParam:@{@"key":@"+/dsf=&",@"name":@"bbbbb"} headerParam:nil customServiceType:0 httpDataType:HttpDataForm complete:^(id response, NSError *error) {
        if (error) {
